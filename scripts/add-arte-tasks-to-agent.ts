...
/**
 * Find agent by name, returns null if not found
 */
async function findAgentByName(
  ...
  const data = await response.json() as { agents: AgentResponse[] };
  return data.agents.find(a => a.name === name) || null;
}

/**
 * Create a new agent (ID is auto-generated by API)
 */
...
async function getActiveVersion(
  ...
  const data = await response.json() as { versions: AgentVersionResponse[] };
  // Find active version
  return data.versions.find(v => v.status === 'active') || data.versions[0] || null;
}
 ...
 *
 * Strategy:
 * 1. If agentId provided, try to find by ID first
 * 2. If not found by ID, search by name "Email Assistant"
 * 3. If still not found, create a new agent
 * 4. Ensure agent has a version with proper prompt template
 */
async function ensureEmailAssistant(
  ...
  if (agentId) {
    agent = await getAgent(agentId, workspaceId, apiBase);
  }

  // If not found by ID, search by name
  if (!agent) {
    console.log(`  Searching for existing "${EMAIL_ASSISTANT_CONFIG.name}" agent...`);
    agent = await findAgentByName(EMAIL_ASSISTANT_CONFIG.name, workspaceId, apiBase);
  }

  // If still not found, create new agent
  if (!agent) {
    console.log(`  Creating Email Assistant agent...`);
...
async function main(): Promise<void> {
  ...
  // Parse CLI arguments
  let agentId: string | null = null; // Will search by name if not provided
  let count = 50;
  ...
  try {
    ...
    console.log(`Agent ID:    ${agentId || '(auto - will find/create "Email Assistant")'}`);
    ...
    // Create taskset
    console.log(`\nStep 2/4: Creating taskset for agent ${actualAgentId}...`);
    ...
    console.log(`     http://localhost:3000/agents/${actualAgentId}/tasksets`);
    console.log('\n  2. Run taskset via API:');
    console.log(`     curl -X POST ${apiBaseUrl}/api/agents/${actualAgentId}/tasksets/${taskset.id}/runs \\`);
